...
#### 必須項目
- ✅ **docstringで指示された内容だけ**を生成し、**指示されていないコードを生成しない**こと。
...
#### キーボード入力の生成ルール
- tkxlibをインポートした場合、キーボード入力には次の関数を使用する：
  - 文字列入力：get_str() -> str
  - 整数入力： get_int() -> int
  ...
  - タプル、セット、辞書の各型入力：それぞれ対応する関数を使用する：
    - `get_tuple`, `get_tuple_int`, `get_tuple_float`, `get_set`, `get_set_int`, `get_set_float`
  - 辞書の各型入力：キーは文字列、値はstr、int, floatのに応じて、それぞれ対応する関数を使用する:
    - `get_dict`, `get_dict_int`, `get_dict_float`
- これらの関数では、指示がない限り**入力プロンプトを生成しない**。
